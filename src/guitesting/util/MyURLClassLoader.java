 ...
 * 
 * @author David Connelly (modified by Gigon Bae)
 * @since 1.2
 */
public abstract class MyURLClassLoader extends SecureClassLoader implements Closeable {
   ...
   * 
   * <p>
   * If there is a security manager, this method first calls the security manager's {@code checkCreateClassLoader}
   ...
   *              if a security manager exists and its {@code checkCreateClassLoader} method doesn't allow creation of a
   *              class loader.
  ...
  MyURLClassLoader(URL[] urls, ClassLoader parent, AccessControlContext acc) {
    super(parent);
    // this is to make the stack depth consistent with 1.1
    SecurityManager security = System.getSecurityManager();
    if (security != null) {
  ...
  /**
   * Constructs a new URLClassLoader for the specified URLs using the default delegation parent <code>ClassLoader</code>
   ...
   * 
   * <p>
   * If there is a security manager, this method first calls the security manager's <code>checkCreateClassLoader</code>
   * method to ensure creation of a class loader is allowed.
   * 
   * @param urls
   *          the URLs from which to load classes and resources
   * 
   * @exception SecurityException
   *              if a security manager exists and its <code>checkCreateClassLoader</code> method doesn't allow creation
   *              of a class loader.
   * @see SecurityManager#checkCreateClassLoader
   */
  ...
  /*
   * A map (used as a set) to keep track of closeable local resources (either JarFiles or FileInputStreams). We don't
   * care about Http resources since they don't need to be closed.
   * 
   * If the resource is coming from a jar file we keep a (weak) reference to the JarFile object which can be closed if
   * URLClassLoader.close() called. Due to jar file caching there will typically be only one JarFile object per
   * underlying jar file.
   * 
   * For file resources, which is probably a less common situation we have to keep a weak reference to each stream.
   */

  private WeakHashMap<Closeable, Void> closeables = new WeakHashMap<>();

  /**
   * Returns an input stream for reading the specified resource. If this loader is closed, then any resources opened by
   * this method will be closed.
   * 
   * <p>
   ...
   * <p>
   * In the case of jar: and file: URLs, it also closes any files that were opened by it. If another thread is loading a
   * class when the {@code close} method is invoked, then the result of that load is undefined.
   * <p>
   ...
   * 
   * @throws IOException
   *           if closing any file opened by this class loader resulted in an IOException. Any such exceptions are
   *           caught internally. If only one is caught, then it is re-thrown. If more than one exception is caught,
   *           then the second and following exceptions are added as suppressed exceptions of the first one caught,
   *           which is then re-thrown.
   * 
   * @throws SecurityException
   *           if a security manager is set, and it denies {@link RuntimePermission}<tt>("closeClassLoader")</tt>
   * 
   * @since 1.7
   */
   ...
   * <p>
   * If the URL specified is <code>null</code> or is already in the list of URLs, or if this loader is closed, then
  ...
  /*
   * Retrieve the package using the specified package name. If non-null, verify the package using the specified code
   * source and manifest.
   */
  ...
  private Class defineClass(String name, Resource res) throws IOException {
    ...
    if (bb != null) {
      ...
      return defineClass(name, instrumentClass(name,bb), cs);
    } else {
      byte[] b = res.getBytes();
      ...
      CodeSource cs = new CodeSource(url, signers);
      sun.misc.PerfCounter.getReadClassBytesTime().addElapsedTimeFrom(t0);
      b = instrumentClass(name,b);
      return defineClass(name, b, 0, b.length, cs);
    }
  }

  /**
   * Appended by Gigon Bae for instrumenting class
   * @param name
   * @param bb
   * @return
   */
  protected abstract ByteBuffer instrumentClass(String name,ByteBuffer bb);
  /**
   * Appended by Gigon Bae for instrumenting class
   * @param name
   * @param b
   * @return
   */
  protected abstract byte[] instrumentClass(String name, byte[] b);

  /**
   * Defines a new package by name in this ClassLoader. The attributes contained in the specified Manifest will be used
   * to obtain package version and sealing information. For sealed packages, the additional URL specifies the code
   ...
   * @return a <code>URL</code> for the resource, or <code>null</code> if the resource could not be found, or if the
   *         loader is closed.
   */
   ...
   * @return an <code>Enumeration</code> of <code>URL</code>s If the loader is closed, the Enumeration will be empty.
   */
   ...
   * <p>
   * If the protocol is "file" and there is an authority component, then permission to connect to and accept connections
   * from that authority may be granted. If the protocol is "file" and the path specifies a file, then permission to
   * read that file is granted. If protocol is "file" and the path is a directory, permission is granted to read all
   * files and (recursively) all files and subdirectories contained in that directory.
   * <p>
  ...
  protected PermissionCollection getPermissions(CodeSource codesource) {
    ...
    if (p instanceof FilePermission) {
      ...
      /**
       * Not loading from a 'file:' URL so we want to give the class permission to connect to and accept from the remote
       * host after we've made sure the host is the correct one and is valid.
       */
      URL locUrl = url;
